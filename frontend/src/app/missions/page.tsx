...
export default function MissionsPage() {
  ...
  const moveUp = useCallback((idx:number) => {
    setMission(m => {
      if(idx===0) return m;
      const wps=[...m.waypoints]; [wps[idx-1],wps[idx]]=[wps[idx],wps[idx-1]];
      return {...m,waypoints:wps};
    });
  },[setMission]);

  const moveDown = useCallback((idx:number) => {
    setMission(m => {
      if(idx>=m.waypoints.length-1) return m;
      const wps=[...m.waypoints]; [wps[idx],wps[idx+1]]=[wps[idx+1],wps[idx]];
      return {...m,waypoints:wps};
    });
  },[setMission]);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <MissionToolbar
        ...
        onTool={setTool} onSamPreset={setSamPreset}
        onAnalyze={analyzeThreat} onClear={clear}
        analyzing={mission.analyzing}
        wpCount={mission.waypoints.length}
        siteCount={mission.sites.length}
        ...
        <div className="w-72 shrink-0 border-l border-border-dim bg-bg-surface flex flex-col overflow-hidden">
          ...
          <div className="p-3 border-b border-border-dim space-y-2">
            ...
            <p className="font-mono text-2xs text-text-dim">
              {mission.waypoints.length} waypoints · {mission.sites.length} threats
              {mission.id && <span className="ml-2 text-text-dim">#{mission.id.slice(0,8)}</span>}
            </p>
            ...
            <div className="flex gap-1.5">
              <button
                onClick={saveMission}
                disabled={mission.saving}
                className={clsx(
                  "flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded border font-mono text-2xs tracking-widest transition-all",
                  mission.saved
                    ? "border-threat-low/40 text-threat-low bg-threat-low/5"
              ...
              <button
                ...
                className={clsx(
                  "flex items-center justify-center gap-1.5 px-3 py-1.5 rounded border font-mono text-2xs tracking-widest transition-all",
                  showLoad
                ...
                {savedLoading ? (
                  ...
                  savedMissions.map(m => (
                    <div key={m.id} className="flex items-center gap-2 px-2.5 py-2 border-b border-border-dim last:border-0 hover:bg-bg-surface transition-colors">
                      <button
                        onClick={() => { loadMission(m); setShowLoad(false); }}
                        className="flex-1 text-left"
                      >
                        <p className="font-mono text-2xs text-text-primary">{m.name}</p>
                        <p className="font-mono text-2xs text-text-dim">
                          {m.waypoints?.length??0} WP · Risk {((m.overall_risk??0)*100).toFixed(0)}%
                        </p>
                      </button>
                      <button
                        onClick={() => deleteMission(m.id)}
                        className="p-1 text-text-dim hover:text-threat-high transition-colors"
                      >
          ...
          <div className="p-3 border-b border-border-dim">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                {label:"SPEED m/s", key:"uav_speed", val:mission.uav_speed, min:1,   max:300, step:1},
          ...
          {mission.overall_risk>0 && (
            <div className={clsx("mx-3 mt-3 p-2.5 rounded border",
              ...
              mission.overall_risk>0.15?"border-threat-medium/40 bg-threat-medium/10":
          ...
          <div className="flex-1 overflow-hidden p-3">
            ...
            <WaypointList
              ...
              onUpdate={updateWaypoint} onRemove={removeWaypoint}

...
export default function MissionsPage() {
  ...
  const moveUp   = useCallback((idx:number) => { setMission(m=>{if(idx===0) return m; const wps=[...m.waypoints];[wps[idx-1],wps[idx]]=[wps[idx],wps[idx-1]];return{...m,waypoints:wps};}); },[setMission]);
  const moveDown = useCallback((idx:number) => { setMission(m=>{if(idx>=m.waypoints.length-1) return m; const wps=[...m.waypoints];[wps[idx],wps[idx+1]]=[wps[idx+1],wps[idx]];return{...m,waypoints:wps};}); },[setMission]);
        ...
        <div className="w-72 shrink-0 border-l border-border-dim bg-bg-surface flex flex-col overflow-hidden">
          ...
          <div className="p-3 border-b border-border-dim space-y-2">
            ...
            <p className="font-mono text-2xs text-text-dim">
              ...
              {mission.id&&<span className="ml-2 opacity-50">#{mission.id.slice(0,8)}</span>}
            </p>
            <div className="flex gap-1.5">
              <button onClick={saveMission} disabled={mission.saving}
                ...
                {mission.saving?<><Loader2 className="w-3 h-3 animate-spin"/>SAVING</>:mission.saved?<><CheckCircle className="w-3 h-3"/>SAVED</>:<><Save className="w-3 h-3"/>SAVE</>}
              </button>
            ...
            {showLoad&&(
              <div className="bg-bg-base border border-border-dim rounded overflow-hidden max-h-48 overflow-y-auto">
                {savedLoading?<div className="p-3 text-center"><Loader2 className="w-4 h-4 animate-spin mx-auto text-text-dim"/></div>
                :savedMissions.length===0?<p className="p-3 font-mono text-2xs text-text-dim text-center">No saved missions</p>
                :savedMissions.map(m=>(
                  <div key={m.id} className="flex items-center gap-2 px-2.5 py-2 border-b border-border-dim last:border-0 hover:bg-bg-surface transition-colors">
                    <button onClick={()=>{loadMission(m);setShowLoad(false);}} className="flex-1 text-left">
                      ...
                      <p className="font-mono text-2xs text-text-dim">{m.waypoints?.length??0} WP · {((m.overall_risk??0)*100).toFixed(0)}% risk</p>
                    </button>
          ...
          {mission.overall_risk>0&&(
            <div className={clsx("mx-3 mt-3 p-2.5 rounded border",
              ...
              <p className={clsx("font-mono text-xl font-medium mt-0.5",mission.overall_risk>0.7?"text-purple-400":mission.overall_risk>0.4?"text-threat-high":mission.overall_risk>0.15?"text-threat-medium":"text-threat-low")}>
          ...
          <div className="flex-1 overflow-hidden p-3">
            ...
            <WaypointList waypoints={mission.waypoints} onUpdate={updateWaypoint} onRemove={removeWaypoint} onMoveUp={moveUp} onMoveDown={moveDown}/>
          </div>
        </div>
      </div>
    </div>

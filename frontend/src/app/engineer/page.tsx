...
import {
  Wrench, AlertTriangle, Zap, ChevronDown, ChevronUp,
  Shield, Radio, Cpu, Wifi, Eye, Wind, Play,
} from "lucide-react";
...
const CAT_ICONS: Record<string,any> = {
  HARDWARE:Cpu, AVIONICS:Shield, COMMS:Wifi,
  PROPULSION:Wind, SENSOR:Eye, STRUCTURAL:AlertTriangle,
};

const UAV_CLASSES = [
  "NANO","MICRO_FPV","STRIKE_FPV","TACTICAL_MULTIROTOR",
  "TACTICAL_VTOL","FIXED_WING_ISR","LOITERING_MUNITION","MALE","HALE",
];
...
function FailureCard({
  ...
  failure:       Failure;
  onInjectToSim: (f: Failure) => void;
}) {
  const [expanded, setExpanded] = useState(false);
  const cfg  = SEV[failure.severity] ?? SEV.MINOR;
  const Icon = CAT_ICONS[failure.category] ?? Wrench;

  return (
    <div className={clsx("border rounded transition-all", cfg.border, cfg.bg)}>
      <button
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-0.5">
            ...
            {!failure.can_continue && (
              <span className="font-mono text-2xs text-threat-high border border-threat-high/30 bg-threat-high/5 px-1.5 py-0.5 rounded">
                LAND NOW
              </span>
            )}
          </div>
          ...
          {/* Quick effect indicators */}
          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            {failure.thrust_loss  > 0 && (
              <span className="font-mono text-2xs text-threat-medium">Thrust −{failure.thrust_loss}%</span>
            )}
            {failure.control_loss > 0 && (
              <span className="font-mono text-2xs text-threat-medium">Control −{failure.control_loss}%</span>
            )}
            {failure.comms_loss   > 0 && (
              <span className="font-mono text-2xs text-threat-medium">Comms −{failure.comms_loss}%</span>
            )}
            {failure.sensor_loss.map(s => (
              <span key={s} className="font-mono text-2xs text-threat-medium">{s} lost</span>
            ))}
          </div>
        </div>
        {expanded
          ? <ChevronUp   className="w-4 h-4 text-text-dim shrink-0" strokeWidth={1.5}/>
          : <ChevronDown className="w-4 h-4 text-text-dim shrink-0" strokeWidth={1.5}/>
        }
      </button>
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3 border-t border-border-dim pt-3">
          ...
          <div>
            <p className="font-mono text-2xs text-text-secondary tracking-widest mb-1.5">
          ...
          {/* Inject to simulator */}
          <button
            onClick={() => onInjectToSim(failure)}
            className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded border border-border-active bg-cyan-subtle text-cyan-DEFAULT font-mono text-2xs tracking-widest hover:shadow-cyan-sm transition-all"
          >
...
export default function EngineerPage() {
  ...
  const simulate = useMutation({
    mutationFn: () => api.post("/engineer/simulate-failure", {
      uav_class:    uavClass,
      flight_hours: flightHrs,
      has_ecm:      false,
    }, token ?? undefined),
  ...
  return (
    <div className="p-4 md:p-5 space-y-5 max-w-[1200px]">
      ...
      <div>
        ...
        <h1 className="font-mono text-base text-text-primary tracking-wide">
      ...
      <div className="bg-bg-surface border border-border-dim rounded p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
          <div>
            ...
            <select
              value={uavClass}
              onChange={e => setUavClass(e.target.value)}
          ...
          <div>
            <label className="font-mono text-2xs text-text-dim block mb-1">
              FLIGHT HOURS: {flightHrs.toFixed(1)}h
            </label>
            <input
              type="range" min={0.1} max={10} step={0.1} value={flightHrs}
              onChange={e => setFlightHrs(Number(e.target.value))}
              className="w-full accent-cyan-500 mt-1.5"
            />
            <div className="flex justify-between mt-0.5">
              <span className="font-mono text-2xs text-text-dim">0.1h</span>
              <span className="font-mono text-2xs text-text-dim">10h</span>
            </div>
          </div>
          ...
          <div className="flex items-end">
            <button
              ...
              className={clsx(
                "w-full flex items-center justify-center gap-2 py-2 rounded border font-mono text-xs tracking-widest transition-all",
                simulate.isPending
            ...
            >
              ...
              {simulate.isPending ? "SIMULATING…" : "INJECT FAILURE"}
            </button>
          </div>
        </div>
        ...
        {simResult && (
          simResult.failure ? (
            <div className={clsx(
              "p-3 rounded border mt-2 space-y-2",
              SEV[simResult.failure.severity]?.border,
              SEV[simResult.failure.severity]?.bg,
            )}>
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-2">
                  <AlertTriangle
                    className={clsx("w-4 h-4", SEV[simResult.failure.severity]?.color)}
                    strokeWidth={1.5}
                  />
                  <span className={clsx(
                    "font-mono text-xs font-medium",
                    SEV[simResult.failure.severity]?.color,
                  )}>
                    FAILURE: {simResult.failure.name}
                  </span>
                </div>
                <button
                  onClick={() => handleInjectToSim(simResult.failure)}
                  className="flex items-center gap-1 px-2.5 py-1 rounded border border-border-active bg-cyan-subtle text-cyan-DEFAULT font-mono text-2xs tracking-widest hover:shadow-cyan-sm transition-all"
                >
                  <Play className="w-3 h-3" strokeWidth={1.5}/>TEST IN SIM
                </button>
              </div>
              <p className="font-mono text-2xs text-text-secondary">{simResult.failure.description}</p>
              <div>
            ...
            <div className="p-3 rounded border border-threat-low/30 bg-threat-low/5 mt-2">
              <p className="font-mono text-2xs text-threat-low">
                ✓ Flight nominal — no failure generated for {flightHrs.toFixed(1)}h on {uavClass}
              </p>
            </div>
          )
        )}
      ...
      <div>
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          ...
          <span className="font-mono text-2xs text-text-dim ml-1">({failures.length})</span>
          ...
          <div className="space-y-2">
            {failures.map(f => (
              <FailureCard
                key={f.id}
                failure={f}
                onInjectToSim={handleInjectToSim}
              />
            ))}
          </div>
        )}

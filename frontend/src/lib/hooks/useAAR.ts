...
export function generateMockAAR(name:string):{events:AAREvent[];metrics:AARMetrics;track:FlightRecord[]}{
  ...
  const events:AAREvent[]=[
    ...
    {time_s:78,type:"THREAT",title:"RADAR DETECTED",detail:"Tor-M1 at 8.2km — tracking initiated",severity:"danger"},
    {time_s:95,type:"THREAT",title:"RADAR LOCK",detail:"Reaction time elapsed — engaged",severity:"danger"},
    {time_s:97,type:"INFO",title:"MISSILE FIRED",detail:"P(k)=0.42 — evade initiated",severity:"warning"},
    {time_s:102,type:"INFO",title:"MISSILE MISSED",detail:"Miss distance 28m — NOE descent",severity:"success"},
    {time_s:140,type:"WAYPOINT",title:"WP2 REACHED",detail:"Alt: 85m, Spd: 18 m/s — NOE",severity:"success"},
    {time_s:185,type:"EW",title:"DATALINK DEGRADED",detail:"Link quality 45% — packet loss 28%",severity:"warning"},
    ...
    {time_s:290,type:"EW",title:"GPS RESTORED",detail:"EW coverage exited — nominal nav",severity:"info"},
    {time_s:340,type:"WAYPOINT",title:"WP4 REACHED",detail:"Alt: 150m, Spd: 22 m/s",severity:"success"},
    {time_s:380,type:"INFO",title:"RETURN TO HOME",detail:"Mission objectives complete",severity:"info"},
...
export function useAAR(){
  const[state,setState]=useState<AARState>({events:[],metrics:null,track:[],playing:false,time_s:0,duration:0});
  const intRef=useRef<NodeJS.Timeout|null>(null);

  const loadMission=useCallback((name:string)=>{
  ...
  return{state,loadMission,play,pause,seek,reset};
}

...
import {
  cacheMission, cacheMissions, getCachedMissions, removeCachedMission, isOnline,
} from "@/lib/offline/missionCache";
...
const DEFAULT: MissionState = {
  id: null, name: "MISSION ALPHA",
  waypoints: [], sites: [],
  uav_rcs: 0.1, uav_speed: 30,
  analyzing: false, overall_risk: 0,
  saved: false, saving: false, offline: false,
};
...
export function useMission() {
  ...
  const addWaypoint = useCallback((lat: number, lon: number) => {
    const wp: Waypoint = {
      id: `wp_${Date.now()}`, lat, lon,
      alt_m: 150, speed_ms: 20,
  ...
  const updateWaypoint = useCallback((id: string, changes: Partial<Waypoint>) => {
    setMission(m => ({
      ...m,
      waypoints: m.waypoints.map(wp => wp.id === id ? { ...wp, ...changes } : wp),
      saved: false,
    }));
  }, []);
  ...
  const addSite = useCallback((lat: number, lon: number, preset = "tor-m1") => {
    const site: ThreatSite = {
      id: `site_${Date.now()}`, name: preset.toUpperCase(),
      lat, lon, preset, alt_m: 0,
    };
  ...
  const analyzeThreat = useCallback(async () => {
    ...
    try {
      const res = await api.post<{ threat_map: any[]; overall_risk: number }>(
        "/sam/mission-threat",
        {
          ...
          uav_rcs_m2:  mission.uav_rcs,
          uav_speed_ms:mission.uav_speed,
        },
        token,
      );
      setMission(m => ({
      ...
      setMission(m => ({ ...m, analyzing: false }));
    }
  ...
  const saveMission = useCallback(async () => {
    ...
    try {
      if (!online) {
        // Offline save — local only with temp ID
        const tempId = mission.id ?? `offline_${Date.now()}`;
        const offlineMission = { ...body, id: tempId, status: "DRAFT", score: 0, created_at: new Date().toISOString(), _offline: true };
        await cacheMission(offlineMission);
        setMission(m => ({ ...m, id: tempId, saving: false, saved: true, offline: true }));
  ...
  const loadMission = useCallback((saved: SavedMission) => {
    setMission({
      ...
      analyzing:    false, saving: false,
      saved:        true, offline: !!(saved as any)._offline,
    });
  }, []);

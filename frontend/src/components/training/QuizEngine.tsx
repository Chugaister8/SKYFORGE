...
import { CheckCircle, XCircle, Clock, Trophy, RotateCcw, ChevronRight, ArrowLeft } from "lucide-react";
...
interface Props {
  ...
  passScore:  number;
  onBack:     ()=>void;
  onComplete: (score:number, grade:string)=>void;
}

export function QuizEngine({courseId,moduleId,moduleTitle,passScore,onBack,onComplete}:Props){
  const{state,questions,start,answer,restart}=useQuiz(courseId,moduleId);
  ...
  if(!state.started) return(
    <div className="flex flex-col items-center justify-center h-full gap-6 p-8">
      <div className="text-center space-y-2">
        ...
        <p className="font-mono text-xs text-text-secondary">{questions.length} questions · {Math.floor(300/60)} min limit</p>
      ...
      <div className="bg-bg-raised border border-border-dim rounded p-4 max-w-sm w-full space-y-2">
        {["Read each question carefully","You cannot go back","Explanation shown after each answer","Timer runs continuously"].map((t,i)=>(
  ...
  if(state.finished){
    ...
    return(
      <div className="flex flex-col items-center justify-center h-full gap-6 p-8">
        <div className={clsx("w-24 h-24 rounded-lg flex items-center justify-center text-5xl font-mono font-bold border-2",g.color,g.bg,g.border)}>
          {state.grade}
        </div>
        <div className="text-center">
          ...
          <p className={clsx("font-mono text-xs mt-2 font-medium",passed?"text-threat-low":"text-threat-high")}>
            {passed?"✓ PASSED":"✗ FAILED"} — minimum {passScore}%
          </p>
        </div>

        {/* Per-question breakdown */}
        <div className="w-full max-w-lg space-y-2 max-h-64 overflow-y-auto">
          {questions.map((q,i)=>{
            const r=state.results[i];
            return(
              <div key={q.id} className={clsx("flex items-start gap-3 p-2.5 rounded border",
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {!r?.correct&&(
                    <p className="font-mono text-2xs text-threat-low mt-1">✓ {q.options[q.correct]}</p>
                  )}
        ...
        <div className="flex gap-3">
          {!passed&&(
            <button onClick={restart} className="flex items-center gap-1.5 px-4 py-2 rounded border border-border-dim text-text-secondary hover:text-text-primary font-mono text-xs transition-all">
              <RotateCcw className="w-3.5 h-3.5"/>RETRY
            </button>
          )}
          <button onClick={()=>onComplete(state.score,state.grade)}
            className={clsx("flex items-center gap-1.5 px-6 py-2 rounded border font-mono text-xs tracking-widest transition-all",
              passed?"border-threat-low bg-threat-low/10 text-threat-low hover:bg-threat-low/20":
              "border-border-dim text-text-secondary hover:text-text-primary")}>
            <Trophy className="w-3.5 h-3.5"/>{passed?"CLAIM SCORE":"BACK TO COURSE"}
          </button>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between p-4 border-b border-border-dim shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="w-32 h-1 bg-bg-base rounded overflow-hidden">
            <div className="h-full bg-cyan-DEFAULT rounded transition-all"
              style={{width:`${(state.current/questions.length)*100}%`}}/>
          </div>
        </div>
        <div className={clsx("flex items-center gap-1.5 font-mono text-xs tabular-nums",
          state.timeLeft<60?"text-threat-high animate-pulse":"text-text-secondary")}>
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
        ...
        {showExplain&&(
          <div className={clsx("p-3 rounded border",
            isCorrect?"border-threat-low/40 bg-threat-low/5":"border-threat-high/40 bg-threat-high/5")}>
            <p className={clsx("font-mono text-2xs font-medium mb-1",isCorrect?"text-threat-low":"text-threat-high")}>
        ...
        {questions.map((_,i)=>(
          <span key={i} className={clsx("w-2 h-2 rounded-full",
            i<state.results.length
              ?state.results[i].correct?"bg-threat-low":"bg-threat-high"

...
const CLASS_LABELS: Record<string, string> = {
  ...
  TACTICAL_VTOL:"VTOL",FIXED_WING_ISR:"ISR",LOITERING_MUNITION:"LM",MALE:"MALE",HALE:"HALE",
};
...
export function FleetList() {
  ...
  if (isLoading) return (
    <div className="space-y-2">{[1,2,3].map((i)=><div key={i} className="h-14 bg-bg-raised rounded animate-pulse"/>)}</div>
  );
      ...
      {fleet.map((uav) => {
        ...
        return (
          <div key={uav.id} className={clsx(
            ...
            "hover:border-border-active bg-bg-raised hover:bg-bg-surface transition-all cursor-pointer",
            uav.status==="IN_MISSION" && "border-cyan-DEFAULT/30 bg-cyan-subtle",
          )}>
            <span className={clsx("w-2 h-2 rounded-full shrink-0",cfg.dot,(uav.status==="ONLINE"||uav.status==="IN_MISSION")&&"animate-pulse-slow")}/>
              ...
              <div className="flex items-center gap-4 flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className={clsx("font-mono text-2xs tabular-nums",
                    telem.battery_pct<30?"text-threat-high":telem.battery_pct<50?"text-threat-medium":"text-text-secondary")}>
                    {telem.battery_pct?.toFixed(0)}%
                  </span>
                </div>
                <div className="flex items-center gap-1 ml-auto">
                  <Wifi className="w-3 h-3 text-threat-low" strokeWidth={1.5}/>
                  <span className="font-mono text-2xs text-text-secondary tabular-nums">{(telem.link_quality*100).toFixed(0)}%</span>
                </div>
              </div>
            ) : (
